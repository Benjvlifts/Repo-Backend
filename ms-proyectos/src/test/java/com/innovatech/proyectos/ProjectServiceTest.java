...
class ProjectServiceTest {
    ...
    class UpdateProjectTests {
        ...
        void updateProject_existingId_updatesAndReturnsResponse() {
            // FIX: UpdateProjectRequest no tiene @Builder (solo @Data/@AllArgsConstructor) -> usar constructor
            UpdateProjectRequest request = new UpdateProjectRequest(
                    "Portal Cliente v2", "Nueva descripción actualizada", null);
    ...
    class AssignEmployeeTests {
        ...
        void assignEmployee_existingProject_setsEmployeeFields() {
            // FIX: AssignEmployeeRequest no tiene @Builder -> usar constructor
        ...
        void assignEmployee_nonExistingProject_throwsException() {
            ...
            assertThatThrownBy(() -> projectService.assignEmployee(99L,
                    new AssignEmployeeRequest(1L, "Test")))
                    .isInstanceOf(IllegalArgumentException.class);
        }
    }
    ...
    class QueryTests {
        ...
        @Test
        @DisplayName("✅ getProjectsByAssignedUser filtra usando el repositorio")
        void getProjectsByAssignedUser_returnsOnlyAssignedProjects() {
            // FIX: ProjectService.getProjectsByAssignedUser ahora delega en
            // projectRepository.findByAssignedUserId(...) en lugar de filtrar
            // en memoria con findAll().
            sampleProject.setAssignedUserId(5L);
    ...
    class NoteTests {
        ...
        void reviewNote_pendingNote_approvesAndSetsReviewer() {
            // FIX: ReviewNoteRequest no tiene @Builder -> usar constructor
            ReviewNoteRequest request = new ReviewNoteRequest(
                    ProjectNote.NoteStatus.APPROVED, "Excelente trabajo");
            ...
            // FIX: reviewNote ahora recibe un 6to parámetro "reviewerRole" para validar permisos.
            // "MANAGER" coincide con sampleProject.managerId=10L, por lo que pasa la validación.
            NoteResponse response = projectService.reviewNote(1L, 1L, request, 10L, "Manager Test", "MANAGER");
        ...
        void reviewNote_alreadyReviewedNote_throwsException() {
            ...
            assertThatThrownBy(() -> projectService.reviewNote(1L, 1L,
                    new ReviewNoteRequest(ProjectNote.NoteStatus.REJECTED, null),
                    10L, "Manager", "MANAGER"))
                    .isInstanceOf(IllegalArgumentException.class)
        ...
        void reviewNote_noteFromDifferentProject_throwsException() {
            ...
            assertThatThrownBy(() -> projectService.reviewNote(1L, 1L,
                    new ReviewNoteRequest(ProjectNote.NoteStatus.APPROVED, null),
                    10L, "Manager", "MANAGER"))
                    .isInstanceOf(IllegalArgumentException.class)
